...
def test_integration():
    ...
    matrix = {
        ...
        2: {
            ...
            "response_times": {
                "per_user": {"Alice": 10.0, "Charlie": 12.0},
                "topic_average": 11.0
            },
    ...
    if personality:
        ...
        for trait in ['openness', 'conscientiousness', 'extraversion', 'agreeableness', 'neuroticism']:
            if trait in personality:
                ...
                print(f"  {trait.title()}: {trait_data.get('mean', 0):.2f} (±{trait_data.get('std', 0):.2f})")
    ...
    for flag in flagged[:3]:  # Show first 3
        print(f"  - Conv {flag.get('conversation_id')}: {flag.get('reason')} (value={flag.get('value', 0):.3f})")

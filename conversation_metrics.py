...
def calculate_response_times(messages: List[Dict]) -> Dict[str, float]:
    ...
    for i in range(1, len(messages)):
        ...
        if (
            current_msg.get("user") != previous_msg.get("user")
            and current_msg.get("datetime")
            and previous_msg.get("datetime")
        ):
...
def calculate_emotional_reciprocity(
    ...
    for i in range(1, len(messages)):
        ...
        current_emotion = (
            (current_sentiment + current_emoji_valence) / 2.0
            if current_emojis
            else current_sentiment
        )
        previous_emotion = (
            (previous_sentiment + previous_emoji_valence) / 2.0
            if previous_emojis
            else previous_sentiment
        )
...
def aggregate_topic_metrics(
    ...
    aggregated = {
        "topic": topic,
        "personality": {"big_five": big_five, "mbti": mbti},
        "emotions": emotion_analysis,
        "response_times": {"per_user": response_times, "topic_average": avg_topic_response_time},

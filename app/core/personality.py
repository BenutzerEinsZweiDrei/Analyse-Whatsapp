...
OPENNESS_INDICATORS = {
    "positive": ["creative", "creative", "idea", "ideas", "imagine", "art", "artistic", 
                 "novel", "curious", "explore", "adventure", "new", "different", "unique",
                 "philosophical", "abstract", "theory", "concept", "innovative"],
    "negative": ["traditional", "conventional", "routine", "practical", "concrete"]
}

CONSCIENTIOUSNESS_INDICATORS = {
    "positive": ["organized", "plan", "planning", "schedule", "prepared", "finish", 
                 "complete", "goal", "goals", "discipline", "careful", "detail", "details",
                 "responsibility", "duty", "achievement", "efficient", "systematic"],
    "negative": ["messy", "disorganized", "chaotic", "spontaneous", "careless", "lazy"]
}

EXTRAVERSION_INDICATORS = {
    "positive": ["party", "social", "friends", "meet", "meeting", "talk", "chat",
                 "energy", "energetic", "excited", "exciting", "fun", "active",
                 "group", "people", "together", "outgoing"],
    "negative": ["alone", "quiet", "shy", "tired", "exhausted", "introvert", "solitude"]
}

AGREEABLENESS_INDICATORS = {
    "positive": ["help", "helping", "kind", "kindness", "care", "caring", "support",
                 "supportive", "friendly", "nice", "compassion", "understanding",
                 "cooperative", "trust", "generous", "sympathetic"],
    "negative": ["conflict", "argue", "argument", "fight", "competitive", "critical", "harsh"]
}

NEUROTICISM_INDICATORS = {
    "positive": ["stress", "stressed", "worry", "worried", "anxious", "anxiety",
                 "nervous", "fear", "afraid", "sad", "depressed", "overwhelmed",
                 "tense", "panic", "upset", "frustrated"],
...
def calculate_big_five_scores(text: str, emojis: List[str]) -> Dict[str, float]:
    ...
    scores["openness"] = 5.0 + min(2.5, openness_pos * 0.5) - min(2.5, openness_neg * 0.5) + min(1.0, emoji_openness * 0.3)
    ...
    scores["conscientiousness"] = 5.0 + min(2.5, consc_pos * 0.5) - min(2.5, consc_neg * 0.5) + min(1.0, emoji_consc * 0.3)
    ...
    scores["extraversion"] = 5.0 + min(2.5, extra_pos * 0.5) - min(2.5, extra_neg * 0.5) + min(1.0, emoji_extra * 0.3)
    ...
    scores["agreeableness"] = 5.0 + min(2.5, agree_pos * 0.5) - min(2.5, agree_neg * 0.5) + min(1.0, emoji_agree * 0.3)
    ...
    scores["neuroticism"] = 5.0 + min(2.5, neuro_pos * 0.5) - min(2.5, neuro_neg * 0.5) + min(1.0, emoji_neuro * 0.3)

...
def run_analysis_multiple_files(
    ...
    for idx, conv_msgs in enumerate(merged_conversations, 1):
        t0 = time.time()
        matrix[idx] = {"idx": idx}
        ...
        # Continue with the same analysis logic as run_analysis
        # (emojis, sentiment, big five, etc.)
        _process_conversation(idx, text, conv_msgs, matrix, empath_lex, emot_obj, settings)

        logger.debug(f"Conversation #{idx} processing time: {time.time() - t0:.2f}s")

    logger.info(f"run_analysis_multiple_files finished in {time.time() - start_time:.2f}s")
...
def run_analysis(file_content: str, username: str) -> Tuple[Dict, Dict]:
    ...
    for idx, conv_msgs in enumerate(conversations, 1):
        ...
        # Process this conversation using shared logic
        _process_conversation(idx, text, conv_msgs, matrix, empath_lex, emot_obj, settings)
